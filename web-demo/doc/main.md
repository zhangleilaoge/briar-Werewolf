...
| [flow.md](flow.md) | 对局流程：回合结构、夜晚、白天、投票、胜利条件 |
...
1. **属性**：六维基础能力，决定检定加值。详见 [numeric.md](numeric.md)。
2. **关系**：有向的信任值与友好值，影响互动与投票。详见 [numeric.md](numeric.md)。
3. **状态**：独立的压力值，影响伪装、观察与情绪行为。详见 [numeric.md](numeric.md)。
4. **阵营**：九宫格行为倾向，修正行动效果与关系/压力变化。详见 [alignment.md](alignment.md)。
...
详见 [flow.md](flow.md)。可用行动详见 [actions.md](actions.md)。
