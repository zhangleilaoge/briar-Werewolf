...
export default function SystemPreview() {
  ...
  const { store, inferences, crisisResult, relations, intentionState } = useMemo(() => {
    ...
    <div className="min-h-screen bg-slate-900 text-slate-200 p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          {activeTab === 'inference' && (
            <>
              <div className="bg-slate-800 rounded-xl p-6 border border-slate-700">
                <h2 className="text-lg font-semibold text-sky-400 mb-4">🎯 角色推理</h2>
              ...
              <div className="bg-slate-800 rounded-xl p-6 border border-slate-700">
                <h2 className="text-lg font-semibold text-sky-400 mb-4">⚠️ 局势推理</h2>
                      ...
                      <div key={c.playerId} className="bg-slate-900 rounded-lg p-3 flex justify-between items-center">
                        <div>
                          ...
                          <div className="flex flex-wrap gap-1 mt-1 text-xs text-slate-400">
                            <span className="bg-slate-800 px-1.5 py-0.5 rounded">指控:{c.factors.accuseCount}</span>
                        ...
                        <div className={`text-xl font-bold ${scoreColor}`}>{c.score}</div>
                      </div>
                    );
                  })}
                </div>
          ...
          {activeTab === 'relation' && (
            <div className="bg-slate-800 rounded-xl p-6 border border-slate-700 lg:col-span-2">
              <h2 className="text-lg font-semibold text-sky-400 mb-4">💕 关系系统（{selectedPlayer} 的视角）</h2>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {relations.getAll().map((rel) => (
                  <div key={rel.playerId} className={`bg-slate-900 rounded-lg p-3 border-l-4 ${rel.friendly > 0 ? 'border-green-500' : rel.friendly < 0 ? 'border-red-500' : 'border-slate-600'}`}>
                    ...
                    <div className={`text-lg font-bold mt-1 ${rel.friendly > 0 ? 'text-green-400' : rel.friendly < 0 ? 'text-red-400' : 'text-slate-400'}`}>
                      {rel.friendly > 0 ? '+' : ''}{rel.friendly}
                    </div>
                ...
                <div className="space-y-1 max-h-48 overflow-y-auto">
                  {intentionState.candidates.slice(0, 8).map((c: ActionCandidate, idx: number) => (
                    <div key={idx} className={`bg-slate-900 rounded-lg p-2 text-sm flex justify-between ${idx === 0 ? 'border border-amber-500/30' : ''}`}>
                      <span><strong>{c.action}</strong>{c.targetId && <span className="text-purple-300"> → {c.targetId}</span>}</span>
