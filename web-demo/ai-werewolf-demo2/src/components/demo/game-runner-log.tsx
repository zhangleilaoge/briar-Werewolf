...
export function generateGameLogs(config: GameConfig): {
  ...
  for (const p of players) {
    store.add({ round: 0, triggerAt: 'init', eventType: 'self_role', actorId: p.id, content: { role: p.role }, source: 'self', credibility: CREDIBILITY.SELF, viewerId: p.id });
  }
  const wolves = players.filter((p) => p.role === 'werewolf');
  for (const w of wolves) {
    for (const t of wolves) {
      if (t.id !== w.id)
        store.add({ round: 0, triggerAt: 'init', eventType: 'teammate_reveal', actorId: w.id, targetId: t.id, content: {}, source: 'system', credibility: CREDIBILITY.SYSTEM, viewerId: w.id });
    }
  }
  ...
  while (!winner && round <= MAX_ROUNDS) {
    ...
    const intentionStates = new Map<string, IntentionState>();
    ...
    for (const self of alive) {
      ...
      intentionStates.set(self.id, intentionState);

      if (selected) {
        switch (selected.action) {
          case 'suspect':
            store.add({ round, triggerAt: 'speech', eventType: 'hear_accuse', actorId: self.id, targetId: selected.targetId, content: {}, source: 'self', credibility: CREDIBILITY.SELF, viewerId: self.id });
            for (const other of alive) { if (other.id !== self.id) store.add({ round, triggerAt: 'speech', eventType: 'hear_accuse', actorId: self.id, targetId: selected.targetId, content: {}, source: 'speech', credibility: CREDIBILITY.SPEECH, viewerId: other.id }); }
            break;
          case 'defend':
            store.add({ round, triggerAt: 'speech', eventType: 'hear_defend', actorId: self.id, targetId: selected.targetId, content: {}, source: 'self', credibility: CREDIBILITY.SELF, viewerId: self.id });
            for (const other of alive) { if (other.id !== self.id) store.add({ round, triggerAt: 'speech', eventType: 'hear_defend', actorId: self.id, targetId: selected.targetId, content: {}, source: 'speech', credibility: CREDIBILITY.SPEECH, viewerId: other.id }); }
            break;
          case 'observe':
            store.add({ round, triggerAt: 'day_start', eventType: 'observe_pattern', actorId: self.id, targetId: selected.targetId, content: { inferredIntention: 'unknown', confidence: 0.5 }, source: 'observe', credibility: CREDIBILITY.OBSERVE, viewerId: self.id });
            break;
          case 'claim_identity':
            store.add({ round, triggerAt: 'speech', eventType: 'hear_claim', actorId: self.id, targetId: selected.targetId, content: { claimedRole: 'prophet' }, source: 'self', credibility: CREDIBILITY.SELF, viewerId: self.id });
            for (const other of alive) { if (other.id !== self.id) store.add({ round, triggerAt: 'speech', eventType: 'hear_claim', actorId: self.id, targetId: selected.targetId, content: { claimedRole: 'prophet' }, source: 'speech', credibility: CREDIBILITY.SPEECH, viewerId: other.id }); }
            break;
          case 'chat':
            store.add({ round, triggerAt: 'speech', eventType: 'hear_chat', actorId: self.id, targetId: selected.targetId, content: {}, source: 'speech', credibility: CREDIBILITY.SPEECH, viewerId: self.id });
            for (const other of alive) { if (other.id !== self.id) store.add({ round, triggerAt: 'speech', eventType: 'hear_chat', actorId: self.id, targetId: selected.targetId, content: {}, source: 'speech', credibility: CREDIBILITY.SPEECH, viewerId: other.id }); }
            break;
        }
      }
    ...
    for (const self of alive) {
      ...
      const visibleMemories = store.getAll().filter((m) => {
        if (m.isForgotten) return false;
        if (m.viewerId && m.viewerId !== self.id) return false;
        return true;
      });
      const forgottenMemories = store.getAll(true).filter((m) => {
        if (!m.isForgotten) return false;
        if (m.viewerId && m.viewerId !== self.id) return false;
        return true;
      });
      ...
      playerResults.set(self.id, {
        intentionState: intentionStates.get(self.id)!,
    ...
    for (const p of alive) {
      ...
      if (bestTarget) {
        ...
        const voterName = p.name;
        const targetName = players.find((x) => x.id === bestTarget)?.name || bestTarget;
        logs.push({ time: nextTime(), playerId: p.id, round, subPhase: 'vote', content: <span>🗳️ <span className="text-amber-400 font-bold">{voterName}</span> 投票给 <span className="text-yellow-300">{targetName}</span></span> });
      }
    }
    ...
    for (const p of prophets) {
      ...
      if (others.length > 0) {
        ...
        store.add({ round, triggerAt: 'night_action', eventType: 'check_result', actorId: p.id, targetId: target.id, content: { result: target.role === 'werewolf' ? 'werewolf' : 'villager' }, source: 'self', credibility: CREDIBILITY.SELF, viewerId: p.id });
        const result = target.role === 'werewolf' ? '狼人' : '村民';
        logs.push({ time: nextTime(), playerId: p.id, round, subPhase: 'night', content: <span>🔮 <span className="text-amber-400 font-bold">{p.name}</span> 查验 <span className="text-yellow-300">{target.name}</span>：{result}</span> });
      }
    }
    ...
    if (aliveWolves.length > 0) {
      ...
      if (nonWolves.length > 0) {
        ...
        const victimName = players.find((x) => x.id === nightKill)?.name || nightKill;
        logs.push({ time: nextTime(), isSystem: true, round, subPhase: 'night', content: <span>🐺 <span className="text-red-400">狼人袭击了 {victimName}</span></span> });
      }
    }
