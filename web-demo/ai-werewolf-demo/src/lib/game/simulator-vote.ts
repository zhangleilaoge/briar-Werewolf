...
import { log, getName, getPublicPlayerStates, logAction } from './simulator-utils';
  ...
  if (topTargets.length === 1) {
    ...
    if (target?.alive) {
      ...
      voters.forEach((voterId) => {
        ...
        if (voter?.alive) {
          sim.playerStateBus.changeRelation(targetId, voterId, { trustDelta: REL_CHANGE_MINOR_NEG, friendlyDelta: REL_CHANGE_MINOR_NEG }, 'vote_exile');
        }
      });
      sim.playerStateBus.killPlayer(targetId, 'vote_exile');
